...
| [BACKEND_DOMAIN_BOUNDARIES.md](docs/backend/reference/BACKEND_DOMAIN_BOUNDARIES.md) | `LIVE` | Maps all 18 modules to 8 bounded contexts and explains why the domain layer exists — the architectural rationale document for the domain structure; read before touching any domain folder | Domain layer rationale (bounded-context structure for growth without monolith collapse), 8 bounded context definitions, full 18-module-to-domain assignment table with activation states, compatibility adapter approach *(created 2026-05-17)* |
| [OPENAPI.yaml](docs/backend/reference/OPENAPI.yaml) | `LIVE` | Machine-readable OpenAPI 3.1 specification for all 27 API routes — the contract for client generation, Swagger UI, and API documentation; served live at GET /v1/openapi.json | 27 paths with request/response schemas, Bearer JWT security scheme, common headers (actor, clientId, workspaceId), reusable schemas (Recommendation, Task, AuditLog, Snapshot, Attribution, BusinessProfile), error response shapes, pagination parameters |
...
| File | Status | Purpose | Description |
|------|--------|---------|-------------|
| [ADR_001_ZERO_RUNTIME_DEPENDENCIES.md](docs/backend/decisions/ADR_001_ZERO_RUNTIME_DEPENDENCIES.md) | `DECISION` | Documents and locks the zero runtime npm dependencies decision — prevents future engineers from casually adding packages without understanding the security and cold-start implications | Context (Render free tier, 512MB RAM, cold-start penalty), decision, rationale (attack surface, cold-start latency, auditability, stability), alternatives considered (Express/Fastify/Hono all rejected), consequences, review trigger conditions |
| [ADR_002_PURE_NODE_HTTP.md](docs/backend/decisions/ADR_002_PURE_NODE_HTTP.md) | `DECISION` | Documents and locks the pure node:http server decision — prevents future engineers from adding Express or Fastify without revisiting the trade-off | Context (HTTP server choice), decision (node:http with explicit if/else routing), rationale (ADR 001 consistency, full control, testability), routing pattern (createRequestHandler → OPTIONS → auth → dispatch → sendEnvelope), alternatives considered, review trigger |
| [ADR_003_IN_MEMORY_DEFAULT_REPOSITORY.md](docs/backend/decisions/ADR_003_IN_MEMORY_DEFAULT_REPOSITORY.md) | `DECISION` | Documents and locks the dual-repository pattern — prevents engineers from removing the in-memory path or adding unnecessary DB requirements for testing | Context (persistence needs, Render spin-down), decision (dual impl per domain, resolved by context.query presence, JS catalog as activation truth), rationale (CI without DB, graceful cold-start), resolveRepository() logic, alternatives considered, review trigger |
| [BACKEND_DOMAIN_SERVICE_ROUTES.md](docs/backend/decisions/BACKEND_DOMAIN_SERVICE_ROUTES.md) | `DECISION` | Documents the decision to expose all 4 domain services via direct HTTP routes — supersedes the original "routeless" decision and provides the complete 26-route inventory as the authoritative reference | History (original routeless decision, routes added 2026-05-15), current 26-route inventory table, per-domain service route detail (measurement, technical-operations, search-intelligence, business-intelligence); historical routeless rationale preserved |
| [BACKEND_DUAL_CLASSIFIER_DECISION.md](docs/backend/decisions/BACKEND_DUAL_CLASSIFIER_DECISION.md) | `DECISION` | Records the intentional existence of two separate intent classifiers — prevents future consolidation attempts by documenting why they must stay at different abstraction layers | core/intentClassifier.js (4-intent heuristic token-based, consumed by search_intent_classifier module) vs domains/search-intelligence/intentClassifier.js (7-intent rule-based, consumed by domain services for SERP analysis); why they must not be merged |
...
| File | Status | Purpose | Description |
|------|--------|---------|-------------|
| [BACKEND_QC_PHASE2.md](docs/backend/implementation/BACKEND_QC_PHASE2.md) | `LIVE` | Quality check audit record proving all 10 Phase 2 expansion modules meet the 6-criterion contract — the verification record for the SEO OS expansion; read to confirm any Phase 2 module was properly built | 6 check criteria (5 files, activation state, flow contract, 3-type tests, createPostgresModuleRunRepository pattern, recordsTable migration match); per-module verdict for all 10 expansion modules; all 10 PASS (60/60) |
| [BACKEND_TEST_HARNESS_REPORT.md](docs/backend/implementation/BACKEND_TEST_HARNESS_REPORT.md) | `LIVE` | Test harness coverage map and CI command reference — read when adding new tests or debugging CI failures to understand which suites exist and what each covers | CI commands (test:backend, test:backend:ci), covered areas (schema/repository, governance, execution lifecycle, scoring, measurement, technical fixtures, search intent, API validation), P0 flow coverage list, no-live-API policy |
| [BACKEND_SCHEMA_REPOSITORY_ALIGNMENT.md](docs/backend/implementation/BACKEND_SCHEMA_REPOSITORY_ALIGNMENT.md) | `LIVE` | Unified schema/repository alignment audit — the authoritative record of how repository field names were aligned to the Supabase migration schema; read before modifying any repository to avoid reintroducing mismatches | Phase 1 (2026-05-06): 5 systemic field-name mismatches fixed across 8 original modules without schema migration; Phase 2 (2026-05-16): all 10 expansion modules confirmed clean; shared persistence.js helper added *(created 2026-05-17)* |
| [BACKEND_EXECUTION_LIFECYCLE_IMPLEMENTATION_REPORT.md](docs/backend/implementation/BACKEND_EXECUTION_LIFECYCLE_IMPLEMENTATION_REPORT.md) | `ARCHIVE` | Implementation record for Issue 03 (Execution Lifecycle) — traces every file, API endpoint, acceptance criterion, and test written for the execution domain; the traceability record for any execution-domain change | Implementation summary of execution domain — recommendations, tasks, status history, audit logs, approval gating, 9 API endpoints |
| [BACKEND_GOVERNANCE_ENGINE_IMPLEMENTATION_REPORT.md](docs/backend/implementation/BACKEND_GOVERNANCE_ENGINE_IMPLEMENTATION_REPORT.md) | `ARCHIVE` | Implementation record for Issue 04 (Governance Engine) — traces all governance files, guardrails, and Google policy anchors built; read when debugging why a recommendation was blocked or verifying which SEO risk policies are enforced | Implementation summary of governance domain — 9 guardrails, 4-level classification (allow / warn / require_approval / block) |
| [BACKEND_MEASUREMENT_ATTRIBUTION_IMPLEMENTATION_REPORT.md](docs/backend/implementation/BACKEND_MEASUREMENT_ATTRIBUTION_IMPLEMENTATION_REPORT.md) | `ARCHIVE` | Implementation record for Issue 05 (Measurement + Attribution Foundation) — documents schema, metric source registry, attribution model, and persistence layer; traceability record for extending measurement or debugging attribution data | Implementation summary of measurement domain — before/after snapshots, attribution links, metric source registry, confidence classification |
| [BACKEND_TECHNICAL_OPERATIONS_IMPLEMENTATION_REPORT.md](docs/backend/implementation/BACKEND_TECHNICAL_OPERATIONS_IMPLEMENTATION_REPORT.md) | `ARCHIVE` | Implementation record for Issue 06 (Technical SEO Backend Foundation) — traces source HTML analyzers and output contracts built; read when extending technical SEO checks or debugging analyzer findings | Implementation summary of technical-operations domain — 11 source HTML analyzers, rendered DOM placeholder contract, analyzer result shape |
| [BACKEND_SEARCH_INTELLIGENCE_IMPLEMENTATION_REPORT.md](docs/backend/implementation/BACKEND_SEARCH_INTELLIGENCE_IMPLEMENTATION_REPORT.md) | `ARCHIVE` | Implementation record for Issue 07 (Search Intelligence Foundation) — documents provider interface, 7-intent classifier, and opportunity scoring built; read when extending SERP analysis or adding new search providers | Implementation summary of search-intelligence domain — 7-intent taxonomy, heuristic classifier, provider interface, opportunity scoring |
| [BACKEND_BUSINESS_INTELLIGENCE_IMPLEMENTATION_REPORT.md](docs/backend/implementation/BACKEND_BUSINESS_INTELLIGENCE_IMPLEMENTATION_REPORT.md) | `ARCHIVE` | Implementation record for Issue 08 (Business Intelligence Layer) — documents business profile model, scoring service, and priority score extension built; read when extending business value inputs or debugging priority ordering | Implementation summary of business-intelligence domain — business profiles, priority score extension, business value scoring |
| [BACKEND_RECOMMENDATION_SCORING_IMPLEMENTATION_REPORT.md](docs/backend/implementation/BACKEND_RECOMMENDATION_SCORING_IMPLEMENTATION_REPORT.md) | `ARCHIVE` | Implementation record for Issue 09 (Recommendation Scoring Engine) — documents the multi-dimensional scoring service and execution lifecycle persistence changes; read when debugging score values or adding new scoring dimensions | Implementation of recommendation scoring engine — 9 scoring dimensions, weighted overall score, priority derivation |
...
| File | Status | Purpose | Description |
|------|--------|---------|-------------|
| [BACKEND_QC_FINAL.md](docs/backend/archive/BACKEND_QC_FINAL.md) | `ARCHIVE` | Final QC verdict and freeze declaration for the original 8-module backend (audit date 2026-04-22) — the authoritative sign-off establishing the V1 baseline; includes scope notice that Phase 2 modules are not covered | Phase 1 QC freeze verdict (10/10) for original 8 modules — dated 2026-04-22; explicitly scoped to Phase 1 only |
| [BACKEND_QC_REPORT.md](docs/backend/archive/BACKEND_QC_REPORT.md) | `ARCHIVE` | Detailed QC evidence report for the original 8-module backend (rerun 2026-04-22) — the per-module evidence log backing the BACKEND_QC_FINAL FREEZE verdict; read when auditing which tests and runtime checks validated each V1 module | Phase 1 QC audit (10/10) for original 8 modules with module pass/fail matrix — dated 2026-04-22 |
| [BACKEND_V1_FROZEN.md](docs/backend/archive/BACKEND_V1_FROZEN.md) | `ARCHIVE` | Formal V1 freeze certificate for the original 8-module backend — the freeze declaration anchoring QC_FINAL and QC_REPORT; read when verifying that V1 was formally frozen before Phase 2 expansion | Phase 1 freeze status record for original 8 modules — dated 2026-04-22; marks V1 as stable before Phase 2 expansion |
| [BACKEND_V1_HARDENED.md](docs/backend/archive/BACKEND_V1_HARDENED.md) | `ARCHIVE` | Validation record confirming the original 8-module backend met the corrected QC rule set before freeze — the pre-freeze hardening certificate; read when verifying which checks ran before the V1 QC sign-off | Phase 1 hardening validation record — checks module catalog, activation, orchestration, tests, HTTP server for original 8 modules |
| [BACKEND_BUILD_SEQUENCE.md](docs/backend/archive/BACKEND_BUILD_SEQUENCE.md) | `ARCHIVE` | Defines the documentation-first and module implementation sequence that governed V1 backend construction — retained as the architectural sequencing record explaining why docs preceded code and why built-but-inactive modules existed from day one | Original Phase 1 build sequence for 8 modules — documentation-first order, architecture setup, implementation sequence; not updated for Phase 2 |
| [BACKEND_IMPLEMENTATION_GAP_SCAN.md](docs/backend/archive/BACKEND_IMPLEMENTATION_GAP_SCAN.md) | `ARCHIVE` | Point-in-time gap scan of V1 implementation against frozen backend docs — records which gaps were fixed and which P1 items remain open (repository factory, concrete providers); read when investigating unresolved persistence wiring | Phase 1 only gap scan for original 8 modules — all declared fixed; superseded by ops/BACKEND_GAP_ANALYSIS_2026_05_16.md which covers all 18 |
| [BACKEND_SCHEMA_REPOSITORY_ALIGNMENT_REPORT.md](docs/backend/archive/BACKEND_SCHEMA_REPOSITORY_ALIGNMENT_REPORT.md) | `ARCHIVE` | Schema/repository alignment report for the original 8-module backend (2026-05-06) — records the 5 systemic field-name mismatches found and fixed without a migration; the V1 Phase 1 alignment record | Phase 1 schema/repository alignment fix for original 8 modules — documents 5 field-name mismatches resolved 2026-05-06; superseded by BACKEND_SCHEMA_REPOSITORY_ALIGNMENT.md |
| [BACKEND_DOMAIN_BOUNDARY_MAP.md](docs/backend/archive/BACKEND_DOMAIN_BOUNDARY_MAP.md) | `ARCHIVE` | Compact domain boundary map created when the domain folder structure was introduced (2026-05-06) — the quick-reference compatibility mapping of all 8 legacy modules to 8 bounded contexts; superseded by BACKEND_DOMAIN_BOUNDARIES.md | Original Phase 1 domain boundary map for 8 modules — fully superseded by BACKEND_DOMAIN_BOUNDARIES.md *(archived 2026-05-17)* |
| [BACKEND_DOMAIN_BOUNDARIES_IMPLEMENTATION_REPORT.md](docs/backend/archive/BACKEND_DOMAIN_BOUNDARIES_IMPLEMENTATION_REPORT.md) | `ARCHIVE` | Implementation record for the domain boundary restructuring (2026-05-06) — traces every domain folder, compatibility adapter, and shared core file created; read when auditing how legacy modules were wired into domain services | Original implementation record for the 8-domain bounded context layer — fully superseded by BACKEND_DOMAIN_BOUNDARIES.md *(archived 2026-05-17)* |
| [BACKEND_SCHEMA_REPOSITORY_ALIGNMENT_PHASE2.md](docs/backend/archive/BACKEND_SCHEMA_REPOSITORY_ALIGNMENT_PHASE2.md) | `ARCHIVE` | Schema/repository alignment audit for the 10 Phase 2 expansion modules (2026-05-16) — confirms all 10 repositories use the shared factory correctly with no deviations; the Phase 2 alignment record complementing the Phase 1 report | Original Phase 2 alignment audit for 10 expansion modules — fully superseded by BACKEND_SCHEMA_REPOSITORY_ALIGNMENT.md *(archived 2026-05-17)* |
...
| File | Status | Purpose | Description |
|------|--------|---------|-------------|
| [FRONTEND_DESIGN_LANGUAGE.md](docs/frontend/reference/FRONTEND_DESIGN_LANGUAGE.md) | `LIVE` | Defines and freezes the premium-minimal visual language governing all UI work — the governing document for visual tone, density, hierarchy, and anti-patterns; consult before any new screen or component work | Frozen design language spec — typography, colour palette, spacing scale, elevation, motion principles; renamed from PHASE_07_DESIGN_LANGUAGE.md to reflect active reference status |
| [FRONTEND_DESIGN_SYSTEM.md](docs/frontend/reference/FRONTEND_DESIGN_SYSTEM.md) | `LIVE` | Translates the design language into enforceable implementation rules for components, spacing, color, and typography — the implementation authority for the design language; consult before building or modifying any shared component | Frozen design system spec — component tokens, grid system, breakpoints, theming rules; renamed from PHASE_08_DESIGN_SYSTEM.md to reflect active reference status |
| [CURRENT_UI_BASELINE.md](docs/frontend/reference/CURRENT_UI_BASELINE.md) | `LIVE` | Authoritative snapshot of the current frontend baseline — overrides older visual assumptions; read at the start of any frontend session to understand what has been built and what the parent-page rules are | Current UI component inventory and screen count — baseline state snapshot for iterative design reference |
| [FRONTEND_MICROCOPY_RULES.md](docs/frontend/reference/FRONTEND_MICROCOPY_RULES.md) | `LIVE` | Defines how all UI text is written — the enforceable writing standard covering the statement→reason→action pattern, length rules, language rules, and action specificity; consult before writing or reviewing any UI copy | Microcopy rules — labels, empty states, error messages, CTA text standards across all screens |
| [FRONTEND_BACKEND_CONTENT_MAPPING.md](docs/frontend/reference/FRONTEND_BACKEND_CONTENT_MAPPING.md) | `LIVE` | Defines how each backend module's outputs map to frontend content layers — the translation contract ensuring raw backend data becomes interpreted decisions; consult before projecting any module output into UI screens | Maps each backend module's output fields to the specific UI elements that display them |
| [FRONTEND_MODULE_FEATURE_MAPPING.md](docs/frontend/reference/FRONTEND_MODULE_FEATURE_MAPPING.md) | `LIVE` | Converts all 18 backend modules into named commercial feature surfaces with commercial jobs, required screen blocks, and demo data requirements — the product feature authority; consult before implementing or updating any module screen | Maps all 18 backend modules to frontend feature surfaces — full commercial job, screen blocks, and demo data for all 18; Phase 2 expansion modules documented with planned feature surfaces *(backfilled 2026-05-17)* |
...
| File | Status | Purpose | Description |
|------|--------|---------|-------------|
| [FRONTEND_MASTER_PLAN.md](docs/frontend/planning/FRONTEND_MASTER_PLAN.md) | `LIVE` | Top-level Flutter frontend build plan — defines product surface, 13-phase sequence, design direction, and dependency policy; the anchor document for the UI system; read at the start of any new frontend work stream | Master frontend plan — full screen list, implementation phases, feature priorities; product surface lists all 18 modules across MVP / gated / Phase 2 expansion / opt-in tiers *(backfilled 2026-05-17)* |
| [FRONTEND_SCREEN_ARCHETYPES.md](docs/frontend/planning/FRONTEND_SCREEN_ARCHETYPES.md) | `LIVE` | Defines the 8 reusable screen archetypes all module screens must compose — structural templates governing layout, behavior, content responsibilities, and anti-patterns; consult before building any new screen type | Screen archetype definitions — the 6 reusable screen patterns used across the app, with composition rules |
| [FRONTEND_BACKEND_CAPABILITY_AUDIT.md](docs/frontend/planning/FRONTEND_BACKEND_CAPABILITY_AUDIT.md) | `LIVE` | Deterministic audit of every implemented backend capability across all 18 modules — the authoritative source frontend features must trace back to; must be consulted before projecting any module capability into UI to avoid inventing non-existent features | Full capability audit for all 18 modules — 117 capabilities extracted from backend code (RA=15, CLI=9, KA=8, RT=7, CA=6, OL=5, CML=4, UWL=7, TSA=9, OPS=10, BI=7, ES=7, SIC=6, SFA=5, TA=6, SAR=8, AI=7, LS=7) *(extended 2026-05-19, T3-31)* |
| [FRONTEND_CAPABILITY_TO_FEATURE_WORKFLOW_MAP.md](docs/frontend/planning/FRONTEND_CAPABILITY_TO_FEATURE_WORKFLOW_MAP.md) | `LIVE` | Turns audited backend capabilities into deterministic frontend feature subpages, workflow steps, and outcomes for all 18 modules — the pre-implementation reference; must be updated after each capability audit and before any UI work begins | Subpage maps for all 18 modules — maps capability→screen→workflow→outcome; 2–5 subpage entries per module *(extended 2026-05-19, T3-31)* |
| [FRONTEND_CONTENT_FULL_SYSTEM.md](docs/frontend/planning/FRONTEND_CONTENT_FULL_SYSTEM.md) | `LIVE` | Applies the content system across all 18 module screens — defines every required content block with input, analysis, insight, explanation, evidence, impact, and action; the screen-level content specification preventing passive dashboards | Authoritative full content system — 18 modules (Phase 1 original 8 + Phase 2 10 new); content blocks for all modules with Input/Analysis/Insight/Explanation/Evidence/Impact/Action/NextStep *(extended 2026-05-19, T3-32)* |
| [FRONTEND_PHASE_INDEX.md](docs/frontend/planning/FRONTEND_PHASE_INDEX.md) | `LIVE` | Review and execution map for all 13 frontend work phases — the navigation index into all phase documents; read to locate the right phase doc before starting any phase-gated frontend work | Index of all 13 frontend design phases with status, deliverable summary, and completion date |
