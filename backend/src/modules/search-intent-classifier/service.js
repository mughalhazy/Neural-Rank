...
const { prioritizeSearchIntentActions } = require("./actions");
const { analyzeSearchIntent, normalizeSearchIntentInput, MODULE_KEY } = require("./analysis");
const { generateSearchIntentInsights } = require("./insights");
const { persistSearchIntentRun } = require("./repository");

const definition = getModuleDefinition(MODULE_KEY);

async function resolveIntegrationInput(context = {}, moduleInput = {}) {
  const normalized = normalizeSearchIntentInput(moduleInput);
...
async function runSearchIntentClassifier(moduleInput = {}, context = {}) {
  ...
  const normalizedInput = normalizeSearchIntentInput(mergedInput);
  const analysisResult = analyzeSearchIntent(normalizedInput);
  const insights = generateSearchIntentInsights(analysisResult);
  const { priorityPayload, actionsPayload } = prioritizeSearchIntentActions(insights);

  const persistence = await persistSearchIntentRun(context, {
  ...
  return {
    ...
    intakeResult: {
      moduleKey: definition.moduleKey,
      status: normalizedInput.keywords.length > 0 ? "accepted" : "accepted_without_keywords",
...
const searchIntentClassifierService = {
  definition,
  run: runSearchIntentClassifier,
};
