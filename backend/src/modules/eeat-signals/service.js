...
const { prioritizeEeatActions } = require("./actions");
const { analyzeEeat, normalizeEeatInput, MODULE_KEY } = require("./analysis");
const { generateEeatInsights } = require("./insights");
const { persistEeatSignalsRun } = require("./repository");

const definition = getModuleDefinition(MODULE_KEY);

async function resolveIntegrationInput(context = {}, moduleInput = {}) {
  const normalized = normalizeEeatInput(moduleInput);
  if (normalized.pages.length > 0 || normalized.trustSignals.length > 0) {
...
async function runEeatSignals(moduleInput = {}, context = {}) {
  ...
  const normalizedInput = normalizeEeatInput(mergedInput);
  const analysisResult = analyzeEeat(normalizedInput);
  const insights = generateEeatInsights(analysisResult);
  const { priorityPayload, actionsPayload } = prioritizeEeatActions(insights);

  const persistence = await persistEeatSignalsRun(context, {
