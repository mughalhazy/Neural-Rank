...
const { prioritizeKeywordActions } = require("./actions");
const { analyzeKeywordInput, normalizeKeywordInput, MODULE_KEY } = require("./analysis");
const { generateKeywordInsights } = require("./insights");
const { persistKeywordAnalysisRun } = require("./repository");

const definition = getModuleDefinition(MODULE_KEY);

async function resolveIntegrationInput(context = {}, moduleInput = {}) {
  const normalizedDirectInput = normalizeKeywordInput(moduleInput);
  if (normalizedDirectInput.keywords.length > 0) {
    return {
      status: "direct_input",
      normalizedPayload: {},
    };
...
async function runKeywordAnalysis(moduleInput = {}, context = {}) {
  ...
  const mergedInput =
    adapterResult?.normalizedPayload && Object.keys(adapterResult.normalizedPayload).length > 0
      ...
      : moduleInput;
  const analysisResult = analyzeKeywordInput(normalizeKeywordInput(mergedInput));
  const insights = generateKeywordInsights(analysisResult);
  const { priorityPayload, actionsPayload } = prioritizeKeywordActions(insights);

  const persistence = await persistKeywordAnalysisRun(context, {
  ...
  return {
    ...
    intakeResult: {
      moduleKey: definition.moduleKey,
      status:
        flow.input.keywords.length > 0 ? "accepted" : "accepted_without_keywords",
    ...
    analysisResult: {
      moduleKey: definition.moduleKey,
      status:
        flow.analysis.keywordCount > 0 ? "analysis_complete" : "analysis_empty",
...
const keywordAnalysisService = {
  definition,
  run: runKeywordAnalysis,
};
