...
function resolveQueryAdapter(context = {}) {
  return context.query || context?.db?.query || context?.pg?.query || null;
}
  ...
  return {
    async saveRun({
      ...
      const productTargetId =
        targetResult?.rows?.[0]?.id || targetResult?.[0]?.id || null;

      const insertRunSql = `
        insert into app_public.creative_messaging_layer_records (
          product_target_id,
          module_key,
          input_payload,
          analysis_payload,
