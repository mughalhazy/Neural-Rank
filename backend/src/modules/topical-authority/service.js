...
const { prioritizeTopicalAuthorityActions } = require("./actions");
const { analyzeTopicalAuthority, normalizeTopicalInput, MODULE_KEY } = require("./analysis");
const { generateTopicalAuthorityInsights } = require("./insights");
const { persistTopicalAuthorityRun } = require("./repository");

const definition = getModuleDefinition(MODULE_KEY);

async function resolveIntegrationInput(context = {}, moduleInput = {}) {
  const normalized = normalizeTopicalInput(moduleInput);
  if (normalized.targetTopics.length > 0 || normalized.existingContent.length > 0) {
...
async function runTopicalAuthority(moduleInput = {}, context = {}) {
  ...
  const normalizedInput = normalizeTopicalInput(mergedInput);
  const analysisResult = analyzeTopicalAuthority(normalizedInput);
  const insights = generateTopicalAuthorityInsights(analysisResult);
  const { priorityPayload, actionsPayload } = prioritizeTopicalAuthorityActions(insights);

  const persistence = await persistTopicalAuthorityRun(context, {
  ...
  return {
    ...
    intakeResult: {
      moduleKey: definition.moduleKey,
      status: normalizedInput.targetTopics.length > 0 ? "accepted" : "accepted_without_topics",
...
const topicalAuthorityService = { definition, run: runTopicalAuthority };
